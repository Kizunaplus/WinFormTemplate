...
namespace Kizuna.Plus.WinMvcForm.Framework.Views
{
    ...
    /// 表示クラス　WebBrowserクラス
    /// </summary>
    abstract class AbstractWebView : WebBrowser, IView
    {
        #region メンバー変数
        /// <summary>
        /// イベント割付データ一覧
        ...
        protected override void OnStatusTextChanged(EventArgs e)
        {
            ...
            String statusText = this.StatusText;
            string parentPath = Path.GetDirectoryName(this.PagePath);
            string webRootPath = "file:///" + Path.GetDirectoryName(parentPath).Replace("\\", "/").Replace(":/", "://").Replace(" ", "%20") + "/";
            if (statusText.StartsWith(webRootPath) == true)
            {
                statusText = statusText.Replace(webRootPath, "");
                statusText = statusText.Replace("#", "");
                eventArgs.Message = statusText;
            }
            else
            {
                eventArgs.Message = statusText;
